...
const verifyConversion = async (req, res) => {
    try {
        ...
        // If the team already has an assigned node, prevent further entries
        if (existingTeam && existingTeam.assignedNode) {
            return res.status(400).json({ status: "This team has already completed the conversion process." });
        }
